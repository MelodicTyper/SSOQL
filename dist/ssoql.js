...
class SSOQL {
     ...
     */
    parseVariableOperation(line, block) {
     ...
     */
    parseSelectOperation(line, block) {
     ...
     */
    parseCountOperation(line, block) {
        const parts = line.trim().split(" ");
     ...
     */
    parseDivideOperation(line, block) {
        const parts = line.trim().split(" ");
        if (parts.length >= 3) {
            block.operations.push({
                type: "DIVIDE",
    ...
    parseSelectStatement(statement) {
        ...
        if (parts.length > 1) {
            ...
            if (conditionsPart.startsWith("(") && conditionsPart.endsWith(")")) {
                const conditions = this.parseConditions(conditionsPart.substring(1, conditionsPart.length - 1).trim());
     ...
     */
    executeOperation(operation, context, data) {
    ...
    executeSelect(operation, data) {
        ...
        if (operation.conditions && operation.conditions.length > 0) {
            filteredData = sourceData.filter((item) => this.evaluateConditions(item, operation.conditions));
        }
    ...
    executeCount(operation, data) {
        ...
        if (operation.conditions && operation.conditions.length > 0) {
            items = items.filter((item) => this.evaluateConditions(item, operation.conditions));
        }
        return items.length;
    }
    /**
     ...
     */
    executeDivide(operation) {
        if (!operation.args || operation.args.length < 2)
            return 0;
     ...
     */
    executeMostFrequent(operation, context, data) {
        let values = [];
